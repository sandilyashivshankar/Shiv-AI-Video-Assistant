...
def transcribe_chunk_sarvam(chunk_path: str) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="sarvam_") as temp_dir:
        for i, start_ms in enumerate(range(0, duration_ms, piece_ms)):
            ...
            _make_sarvam_piece(chunk_path, start_ms // 1000, duration_seconds, piece_path)
...
def transcribe_all(chunks: list, language: str = "english") -> str:
    """Transcribe audio chunks, or directly consume a YouTube transcript fallback."""
    if not chunks:
        return ""

    # audio_processor.py may return a .txt file when YouTube blocks media but
    # public captions are available. Do not send that text through Whisper.
    transcript_files = [
        ...
        for path in transcript_files:
            with open(path, "r", encoding="utf-8") as f:
                ...
                if text:
                    texts.append(text)
        if texts:
            print("Using YouTube transcript fallback; audio transcription skipped.")
